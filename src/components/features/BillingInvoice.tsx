      ...
      <div className="print-section">
        ...
        <div className="grid grid-cols-4 gap-3 mb-5">
           <div className="bg-gray-50 p-2.5 rounded-lg border border-gray-200/70">
               ...
               <span className="text-xs font-bold text-gray-800">{vendorProfile?.city || '-'} {vendorProfile?.state || ''}</span>
           </div>
           <div className="bg-gray-50 p-2.5 rounded-lg border border-gray-200/70">
        ...
        <div className="grid grid-cols-2 gap-6 mb-5">
          <div className="space-y-1.5">
            ...
            <div className="text-xs space-y-0.5 leading-snug">
              <p className="font-bold text-gray-900 text-sm">{vendorProfile?.businessName}</p>
               <p className='text-gray-600 font-medium'>{vendorProfile?.email}</p>
               <p className='text-gray-600 font-medium'>+91 {vendorProfile?.mobile}</p>
               <p className="text-gray-600 font-medium">{vendorProfile?.address}</p>
               ...
               {vendorProfile?.gstNumber && (
                 <div className="pt-1.5 flex items-center gap-1.5">
                   ...
                   <span className="font-mono text-gray-800 font-bold tracking-tight">{vendorProfile.gstNumber}</span>
                 </div>
               )}
            </div>
          </div>
          <div className="space-y-1.5 text-right flex flex-col items-end">
            ...
            <div className="text-xs space-y-0.5 leading-snug text-right">
              <p className="font-bold text-gray-900 text-sm mb-0.5">{customer?.name || data?.userName || 'Customer'}</p>
               <p className="text-gray-600 font-medium">{customer?.email || data?.userEmail || 'N/A'}</p>
               <p className="text-gray-600 font-medium">{customer?.phone || customer?.mobile || data?.userPhone || 'N/A'}</p>
               {data.shippingAddress ? (
                 <p className="text-blue-600 pt-1 text-xs font-medium leading-tight max-w-[200px]">{data.shippingAddress}</p>
               ) : (
