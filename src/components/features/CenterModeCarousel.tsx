            ...
            <div className="w-full h-full rounded-3xl overflow-hidden relative shadow-2xl bg-gray-900">
              ...
              <div className="relative z-10 h-full flex flex-col items-center justify-center text-center text-white px-4 md:px-20 max-w-4xl mx-auto">
                <motion.div
                  ...
                  className={`text-sm md:text-base font-bold mb-6 px-4 py-1.5 rounded-full ${BANNERS[activeIndex].color}`}
                >
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex gap-2">
          {BANNERS.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${index === activeIndex ? 'bg-white w-8' : 'bg-white/40 w-2 hover:bg-white/60'
                }`}
            />
          ))}
        </div>
